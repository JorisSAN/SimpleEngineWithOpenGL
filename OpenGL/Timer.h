...
public:
	...
	// Compute delta time as the number of miliseconds since last frame
	// Clamp it so debug won't think delta time is really high during breakpoints
	unsigned int computeDeltaTime();

	// Wait if the game run faster than decide FPS
	void delayTime();
...
private :
	...
	// Time in miliseconds when frame starts
	unsigned int frameStart;

	// Last frame start time in miliseconds
